...
namespace SWP391Web.API.Controllers
{
    ...
    public class VehicleDeliveryController : ControllerBase
    {
        ...
        public async Task<ActionResult<ResponseDTO>> GetAllVehicleDeliveries([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10, DeliveryStatus? status = default, CancellationToken ct = default)
        {
            var response = await _vehicleDeliveryService.GetAllVehicleDelivery(pageNumber, pageSize, status, ct);
